...
void broadcast_cur_time() {
  while (true) {
    ...
    //unsigned long while_loop_ms = cur_time_ms + 15000;
    serial_printf("<*time:%lu:>\n", cur_time_ms);
    delay(2000);
    serial_printf("<*time:%lu>\n", millis());
    delay(2000);
      //while (cur_time_ms <= while_loop_ms) {
    for (int i=0; i<10; i++) {
      //mesure time here
...
void loop() {
  ...
  */
  ...
  //cur_ms = millis();
  //if (time_passed(3000)) {
    //serial_printf("Every three seconds\n");
  //}
  //cur_ms = millis();
  //if (time_passed(last_light_ms, 5000)) { //so, every 5 seconds, also reset last checked value as this is last check
    //broadcast_cur_time();
    //last_light_ms = cur_ms;
  //}
}
